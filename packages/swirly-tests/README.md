...
Every specification in `examples/` is parsed and rendered to SVG under both the
light and dark themes, then compared byte-for-byte against a committed
rendering in `golden/`. Swirly's only real output is pixels, so these snapshots
exist to make layout refactors safe: any change to geometry, styling or
serialization shows up as a failing test rather than as a diagram that quietly
looks wrong.
